...
/// CRTP specialization of MPMCQueueBase
        ...
        class MPMCQueueBase<Derived<T, Atom, Dynamic>> {
            ...
            // MPMCQueueBase是MPMCQueue的基类,使用CRTP(曲柄模式)实现。
            // 主要包含:
            // 1. 构造函数、移动构造、移动赋值运算符的实现。
            // 2. 队列状态查询函数:size()、isEmpty()、isFull()等。
            // 3. 统计写入/读取次数的函数:writeCount()、readCount()。
            // 4. 阻塞式写入/读取函数:blockingWrite()、blockingRead()。
            // 5. 非阻塞式写入/读取函数:write()、read()。
            // 6. 有条件阻塞式写入/读取函数:writeIfNotFull()、readIfNotEmpty()。
            // 7. 按ticket写入/读取的函数:enqueueWithTicket()、dequeueWithTicket()。
            // 8. 获取非阻塞式和有条件阻塞式ticket的函数:tryObtainReadyXXXTicket()、tryObtainPromisedXXXTicket()。
            // 9. 一些辅助函数:计算索引、轮数等。
            // 10. 单元素队列SingleElementQueue声明。
            // 基类实现了队列的基本接口和功能,派生类MPMCQueue实现具体的写入和读取逻辑。
            // 使用CRTP避免虚函数调用开销,static_cast直接调用派生类函数。
        ...
        protected:
            ...
            union {
                ...
                // 队列的数组实现,数组元素是 SingleElementQueue
                Slot *slots_;
            ...
            // 在 MPMCQueue 中,tickets 指的是用于协调对队列的访问的计数器。其中主要有两个:
            // 1. pushTicket_: 用于给写入线程分配计数器,表示可以写入队列。
            // 2. popTicket_: 用于给读取线程分配计数器,表示可以读取队列。
            // 其工作原理如下:
            //- 写入线程会先取得一个 pushTicket,然后用这个 ticket 在队列相应的位置写入元素。
            //- 读取线程会先取得一个 popTicket,然后用这个 ticket 从队列相应的位置读取元素。
            //- ticket 的值是单调递增的。队列会根据 ticket 值计算出应该访问的队列位置。

            // 在 MPMCQueue 中,ticket值到队列位置的计算逻辑如下:
            // 1. queue 内部有一个 slots_ 数组,数组大小为 capacity。
            // 2. 每个线程在执行 enqueue 或 dequeue 操作时,需要先获取一个 ticket。
            // 3. ticket 是一个全局递增的序列号。
            // 4. 将 ticket 对应的队列位置计算为:
            //    idx = (ticket * stride) % capacity
            //    这里 stride 是一个常数,用于使不同线程访问不同的 cache line,避免伪共享。
            // 5. 也就是 ticket 通过取模后映射到 slots_ 数组的某个位置。
            // 6. 之后线程就使用这个位置进行 enqueue/dequeue 操作。
            // 7. 不同线程拿到不同的 ticket,可以映射到 slots_ 的不同位置,从而实现并发。
            // stride 的作用是使不同线程尽量访问不同的 cache line,进一步减少伪共享问题。
            // 所以通过 ticket 定位到队列位置,并使用 stride 调整,可以有效实现一个并发无锁队列。

            //- 队列的每个位置上,会根据 ticket 计算出一个 turn 序号,用于协调对该位置的访问顺序。
            // 所以 tickets 起到一个计数器的作用,用于标识和协调不同线程对队列的操作顺序。
            // 它实现了对整个队列的并发控制,使得不同线程可以无竞争地访问队列,从而实现了一个 MPMC 的无锁队列。
            // tickets 机制也带来了队列操作的线性一致性,写入肯定线性早于后续的写入,读取线性早于后续读取。
            ...
            // 自适应自旋次数
            // 1. 当队列满时,写入线程调用enqueue前会先自旋等待,而不是直接阻塞。
            // 2. pushSpinCutoff_记录自旋的次数,当自旋次数超过它,才会阻塞线程。
            // 3. 它的值会动态调整,以实现自适应自旋。
            alignas(
...
/// used by SingleElementQueue are doubled inside the TurnSequencer
        template<typename T, template<typename> class Atom>
        struct SingleElementQueue {
            // 这是单元素队列 SingleElementQueue 的实现,它管理每个队列槽位(slots数组的元素)的访问。
            // 主要功能:
            // 在contents_中使用placement new构造元素。
            // sequencer_记录每个元素的访问轮次,偶数轮次是写入,奇数轮次是读取。
            // enqueue/dequeue 接口支持写入和读取元素。
            // 支持按轮次等待可写入/可读取。
            // 根据元素类型特性选择复制还是移动语义实现写入/读取。
            // 关键点:
            // 每个元素一个 sequencer 记录访问轮次,控制对该元素的独占访问。
            // 写入时等待偶数轮,读取时等待奇数轮。
            // 写入通过构造函数或移动语义写入,读取通过复制或移动语义读取。
            // 如果元素可重定位,则读使用 memcpy 复制,优于移动语义。
            // 所以它通过轮次机制确保每个槽位元素的互斥访问,可以看作是一个单元素的阻塞队列。
            // 外部通过分配访问轮次实现对整个队列的并发访问调度。
        ...
        private:
            ...
            // 用于存储队列元素的内存空间
            // 通过 placement new 构造元素
            // 1. contents_ 只是一个对齐的内存空间,初时不具任何类型信息。
            // 2. 通过在 contents_ 上调用 placement new T(args),在这个内存空间构造一个 T 类型元素。
            // 3. 此时 contents_ 就持有一个完全构造的 T 元素。
            // 4. 后续可以通过转型为 T* 来访问这个元素。
            // 5. 当要销毁元素时,调用 ptr()->~T() 来执行其析构函数。
            // 6. contents_ 继续作为一段对齐的内存空间,可以再次通过 placement new 构造新的元素
            // 所以 contents_ 就是一段可以通过 placement new 动态构造/析构队列元素的内存空间
            // aligned_storage_for_t 确保它有正确对齐,以支持可能需要对齐的 T 类型。
            // 这种方式可以避免直接持有 T 类型导致的构造/析构开销。
            aligned_storage_for_t<T> contents_;

            /// Even turns are pushes, odd turns are pops
            // 1. 记录每个元素的访问轮次,奇数轮是出队,偶数轮是入队。
            // 2. 提供 waitForTurn() 和 completeTurn() 接口。
            //    waitForTurn() 会一直等待,直到轮到自己的指定轮次。
            //    completeTurn() 表示自己的这个访问轮次完成。
            // 3. 提供 isTurn() 接口查询当前是否是指定的轮次。
            // 4. 提供 tryWaitForTurn() 接口,可以设置超时时间,在 deadline 前等待指定轮次。
            // equencer_ 记录了每个元素的访问轮次,并控制着对每个元素的互斥访问
            // sequencer_ 的机制保证了每个元素都可以被互斥访问,既实现了线程安全,又通过轮次避免了锁的开销
