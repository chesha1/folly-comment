...
// Baton 允许线程阻塞一次并被唤醒：它捕获一次切换。
// 它本质上是一个（非常小，非常快）信号量，
// 仅支持对 sem_call 和 sem_wait 的单个调用。
...
///
    ...
    public:
        ...
        // 提供了一个非阻塞的方式来检查状态是否已经被 post
        // 检查加载的状态是否等于 EARLY_DELIVERY 或 LATE_DELIVERY
        // 如果匹配其中任何一个，函数返回 true，
        // 否则，它返回 false
        ...
        void post() noexcept {
            if (!MayBlock) {
                ...
                // 原子操作安全地将 state_ 的值更新为 EARLY_DELIVERY
        ...
        void wait(const WaitOptions &opt = wait_options()) noexcept {
            // 调用 try_wait() 函数尝试进行非阻塞的等待。
            // 如果 try_wait() 成功，则函数直接返回
            if (try_wait()) {
                return;
            }
            ...
            // 如果 try_wait() 没有成功，这里调用 tryWaitSlow 函数，
            // 并传入上面定义的无限 deadline 和提供的 opt 选项。
            // 这意味着在 tryWaitSlow 中会使用一个慢速（可能是阻塞的）方法进行等待，
            // 直到 deadline 时间点
